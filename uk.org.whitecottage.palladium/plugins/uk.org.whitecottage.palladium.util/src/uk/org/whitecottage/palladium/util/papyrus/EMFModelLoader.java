...
public class EMFModelLoader {
     ...
     * @return the model
     */
    public EObject getModel(URI path) throws Exception
    {
        checkPath(path);
        if( paths == null){
            ...
            switch (resource.getContents().size())
            {
            	case 0:
            		if (!resource.isLoaded())           			
            			throw new Exception("Model can not be loaded from URL: \"" + path.toString() + "\"");
            		break;
            	case 1:
            		isRootVirtual = false;
     ...
     */
    protected void checkPath(URI path) throws Exception
    {
        if (path == null)
        {
     ...
     */
    public EObject getCurrentElement(String elementPath, EObject model, int i, String attribute) throws Exception
    {
        // Check parameters
        if (i < 1)
        {
        ...
        throw new Exception(elementPath.concat(" ").concat(model.eResource().getURI().toString()));
    }
